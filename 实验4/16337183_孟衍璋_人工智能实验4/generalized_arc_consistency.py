...
def gac(remaining,puzzle):
	...
	if judge:
		...
		end_time = time.time()
		write_solution('solution_gac.txt', puzzle)
		write_performance('performance_gac.txt', start_time, end_time, search_start_time, search_end_time, count)
		# sys.exit()
		return True
	...
	for i in remaining[x][y]:
		...
		if gac(new_remaining,new_puzzle) == 0:
			# 如果后面的递归搜索不满足要求，令new_puzzle[i][j] = 0
			new_puzzle[x][y] == 0
		else:
			return True

	# 如果该点遍历之后都不符合要求，则表示上游选值不当，回溯 
	return False
